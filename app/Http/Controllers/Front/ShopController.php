...
class ShopController extends Controller
{
    ...
    public function getCashbackDetail()
    {
        $api = new API();
        return view('front.shops.index', [
            'items' => Shop::orderBy('created_at', 'desc')->paginate(config('pw-config.shop.page')),
    ...
    {
        // Ensure the promo_code is uppercase if your database entries are stored in uppercase
        $promoCode = strtoupper($request->input('promo_code'));
    
        // Check if an active promo code exists
        $exists = PromoCode::where('promo_code', $promoCode)
                            ->where('status', 'active')
                            ->exists();
