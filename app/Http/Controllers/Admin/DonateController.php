...
class DonateController extends Controller
{
    ...
    public function showTripay()
    {
        return view('admin.donate.tripay');
    }

    public function postTripay(Request $request)
    {
        if ($request->has('status')) {
            Config::write('tripay.status', true);
        } else {
            Config::write('tripay.status', false);
        }

        if ($request->has('sandbox')) {
            Config::write('tripay.sandbox', true);
        } else {
            Config::write('tripay.sandbox', false);
        }

        if ($request->has('double')) {
            Config::write('tripay.double', true);
        } else {
            Config::write('tripay.double', false);
        }
        ...
        if (config('tripay.status') === true) {
            $configs = $request->validate([
                ...
                'maximum' => 'required|numeric',
                'refid' => 'required|string',
                'bonusess' => 'nullable|numeric|max:100',
                'mingetbonus' => 'nullable|numeric',
                'route' => 'required|string|min:6|max:30',
                'returnUrl' => 'required|string',
                'list' => 'required|string|regex:/^\d+(,\s*\d+)*$/',
            ]);
            if ($configs['maximum'] < $configs['minimum']) {
                return redirect()->back()->with('error', __('donate.tripay.error.smalmax'));
            } else {

                foreach ($configs as $config => $value) {
                    if (!$value) {
                        Config::write('tripay.' . $config, '');
                    } else {
                        Config::write('tripay.' . $config, $value);
                    }
                }
            }

            // Optional: You can write the 'config-list' array to config as well if needed
            // Config::write('tripay.list', $configList);  // Save 'config-list' as array
        }
    ...
    {
        $history = DB::table('pwp_ipaymu')
            ->leftJoin('users', 'pwp_ipaymu.user_id', '=', 'users.ID')
            ->select('users.*', 'pwp_ipaymu.*')
