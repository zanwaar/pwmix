            ...
            <div class="col-md-8">
                <div class="clearfix">
                    <h3 class="h2 pull-left m-0"><a href="{{ route('show.article', $article->slug ) }}">{{
                            strtoupper($article->title) }}</a></h3>
                    <span class="date pull-right"><i class="fa fa-calendar"></i>{{
                        \Carbon\Carbon::parse($article->created_at)->translatedFormat('j F, Y') }}</span>
                </div>
                <div class="embed-item">
                    ...
                    <span class="label label-{{ $article->categoryColor($article->category) }}">{{ __('news.category.' .
